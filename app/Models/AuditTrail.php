...
*/
        ...
        if (!empty($data_array['search_text'])) {
            ...
            //$model->whereBetween('created_at', ["STR_TO_DATE('" . date("Y-m-d", strtotime($date_split_array[0])) . " 00:00" . ", '%Y-%m-%d %H:%i:%s')", "STR_TO_DATE('" . date("Y-m-d", strtotime($date_split_array[1])) . " 23:59:59" . ",'%Y-%m-%d %H:%i:%s')"]);
        ...
        $results = \DB::select("
        select * from `audit_trails` where (`created_by` = '" . $data_array['users_id'] . "' and `status` = '" . config('constant.STATUS_ACTIVE') . "')  " . $searchQuery . "  and `audit_trails`.`deleted_at` is null order by `created_at` desc
        ");
