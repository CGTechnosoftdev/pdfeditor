...
trait BaseModelTrait
{
     ...
     * @return [type]                [description]
     */
    public static function dataList($condition = array(), $active_only = true)
    {
        $model = self::query();
        if (!empty($active_only)) {
            $model->where('status', config('constant.STATUS_ACTIVE'));
        }
        if (!empty($condition)) {
     ...
     * @return [type]     [description]
     */
    public static function dataRow($condition = array(), $active_only = true)
    {
        $model = self::query();
        if (!empty($active_only)) {
            $model->where('status', config('constant.STATUS_ACTIVE'));
